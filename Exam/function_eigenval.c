...
// Here we have the functions borrowed from the Eigenvalue-exercise
...
int jacobi_classic(gsl_matrix* A, gsl_vector* e, gsl_matrix* V){ // This function is made just as previously in the eigenvalue exercise C.
// It does the Classic Jacobi diagonalization, modified to go from equation to equation starting from the top, returning a upper triangle of A which is destroyed, while e and V accumulate eigenvalues and eigenvectors.

int changed, sweeps=0, n=A->size1; // Initialised values
int maximum[n]; // We set a array for the maximum value on each row.

for(int i=0;i<n;i++)gsl_vector_set(e,i,gsl_matrix_get(A,i,i)); // We copy the diagonal of A to the eigenmatrix
gsl_matrix_set_identity(V); // We define identity matrix as a start for the eigenvectormatrix.

for(int i = 0; i < n; i++){
  maximum[i] = find_max_in_row(A,i); // We find the largest element in each row
}
...
do{
  ...
  int p,q;
    ...
    q=maximum[p]; // We make shure to take only the largest q value in each row
		...
		double app1=c*c*app-2*s*c*apq+s*s*aqq; // The transformation acts on the eigenvalues
		double aqq1=s*s*app+2*s*c*apq+c*c*aqq; // The transformation acts on the eigenvalues
...
int find_max_in_row(gsl_matrix* A, int r){ // This function is made just as previously in the eigenvalue exercise C.
  // We find the maximum in each row by comparing the elements one by one
	int n=A->size1;
  int index_max=0;
